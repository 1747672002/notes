    ...
    private static function getCurlObject($url,$postData=[],$header=array()){
    ...
    private static function request($chList){
        ...
        do {
            ...
            while ($done = curl_multi_info_read($downloader)){
                ...
                //$output = curl_multi_getcontent($done);
     ...
     * 根据传入的数组，foreach循环生成curl对象，并发起请求
    ...
    public static function get_demo($urlArr){
        ...
        if (!empty($urlArr)) {
            ...
            foreach ($urlArr as $key => $url) {
                ...
                $chList[] = self::getCurlObject($url['url'],$url['data']);

            }

            // 将交给 request 方法，批量发起请求
